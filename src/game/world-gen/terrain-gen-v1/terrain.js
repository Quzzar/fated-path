...
import {
  makeRandomLanguage,
  makeName,
} from './dependencies/language.js';
import { data_exports as d3 } from './dependencies/d3.v4.js';
import PriorityQueue from './dependencies/priority-queue/priorityQueue.js';
...
function visualizeVoronoi(svg, field, lo, hi) {
  ...
  for (let i = 0; i < field.mesh.tris.length; i++) {
    let voronoi = field.mesh.tris[i];
    svg.push({
      type: 'path',
      subtype: 'path.field',
      data: {
        d: makeD3Path(voronoi),
        fill: d3.interpolateViridis(mappedvals[i]),
      },
    });
  }
...
function visualizeCities(svg, render) {
  ...
  for (let i = 0; i < cities.length; i++) {
    ...
    svg.push({
      ...
      subtype: 'circle.city',
      data: {
        cx: 1000 * h.mesh.vxs[city][0],
        cy: 1000 * h.mesh.vxs[city][1],
        r: (i >= n ? 4 : 10),
      },
    });
  }
...
function drawLabels(svg, render) {
  ...
  for (let citylabel of citylabels) {
    svg.push({
      type: 'text',
      subtype: 'text.city',
      data: {
        x: 1000 * citylabel.x,
        y: 1000 * citylabel.y,
        fontSize: citylabel.size,
        textAnchor: citylabel.align,
        text: citylabel.text,
      },
    });
  }
...
export async function doMap() {
  ...
  return {
    ...
    /*
    width: width,
    height: width * params.extent.height / params.extent.width,
    viewBox: -1000 * params.extent.width / 2 + ' ' +
